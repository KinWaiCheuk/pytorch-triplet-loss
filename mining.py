...
#     print("frac_pos = {}".format(fraction_positive_triplets))
...
def _get_triplet_mask(labels):
    ...
    indices_equal = torch.eye(labels.size()[0]).type(torch.ByteTensor).to(device) # make a index in Bool
    ...
    distinct_indices = distinct_indices.cuda() # somehow it is in cpu
...
def _get_anchor_positive_triplet_mask(labels):
    ...
    indices_equal = torch.eye(labels.size()[0]).type(torch.ByteTensor).to(device)
    indices_not_equal = -(indices_equal-1) # flip booleans
